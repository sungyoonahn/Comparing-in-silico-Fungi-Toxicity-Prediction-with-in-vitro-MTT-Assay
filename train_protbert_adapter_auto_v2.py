...
def sample_infer_df(df):
    ...
    secretion_random_df = secretion_df.sample(sample_num)
    resistant_random_df = resistant_df.sample(sample_num)
    toxin_random_df = toxin_df.sample(sample_num)
    anti_toxin_random_df = anti_toxin_df.sample(sample_num)
    virulence_random_df = virulence_df.sample(sample_num)
...
if __name__ == "__main__":
    ...
    for i in range(num_runs):
        ...
        # main_train_csv_folder_path = args.datasets_dir+"Uniprot_"+date+"_"+str(num_per_class)+"/"
        # main_output_save_folder_path = args.results_dir+"Uniprot_"+date+"_"+str(num_per_class)+"/"
        main_train_csv_folder_path = "datasets/Swissprot_TrEMBL_0306/"
        main_output_save_folder_path = "results/Swissprot_TrEMBL_0306/"
        ...
        # The below file is the TREMBL data file
        infer_orig_df = pd.read_csv(args.datasets_dir+"TREMBL(UNIPROT)/Bacteria/TREMBL_1008_5_class_trainable.csv")
        #below line is to reduce infer time, only used in training
        print(infer_orig_df.dtypes)
        infer_df = sample_infer_df(infer_orig_df)
        ...
        print(infer_results)
        infer_results.to_csv(save_dir + "TREMBL_1008_5_class_trainable_classification_results_"+date+"_"+str(num_per_class)+".csv", index=False)
        ...
        prev_num_per_class = num_per_class
        ...
        save_new_pth = args.datasets_dir+"Uniprot_"+date+"_"+str(num_per_class)+"/"
        results_save_new_pth = args.results_dir+"Uniprot_"+date+"_"+str(num_per_class)+"/"
        ...
        df = pd.read_csv(args.datasets_dir+"TREMBL(UNIPROT)/Bacteria/TREMBL_1008_5_class_trainable.csv")
        
        df_compare = pd.read_csv(preds_save_pth+"TREMBL_1008_5_class_trainable_classification_results_"+date+"_"+str(prev_num_per_class)+".csv")
        # df_compare = df_compare.drop(['Unnamed: 0'], axis=1)
        ...
        df_new.to_csv(preds_save_pth+"TREMBL_1008_5_class_trainable_classification_results_"+date+"_"+str(prev_num_per_class)+".csv", index=False)
        df = pd.read_csv(preds_save_pth+"TREMBL_1008_5_class_trainable_classification_results_"+date+"_"+str(prev_num_per_class)+".csv")
        ...
        df_Swissprot = pd.read_csv(args.datasets_dir+"Swissprot/Swissprot_1008_6_class_trainable.csv")
        df_prev = join_train_data(preds_data_pth)
        
        print(df_prev)
        print("5. Label count of input trainable data(train, valid, test)")
        check_labels_count(df_prev)

        df_trainable = drop_duplicates(df_Swissprot, df_prev)
        print("6. Label count where label matches predictions")
        check_labels_count(df_preds_matching)
        df_preds_matching = drop_duplicates(df_preds_matching, df_prev)
        ...
        sampled_trainable_df, empty_class_dict = sample_trainable_df_5class(df_trainable, update_data_num)
