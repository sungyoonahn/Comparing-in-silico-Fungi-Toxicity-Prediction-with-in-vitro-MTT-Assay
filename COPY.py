...
def sample_trainable_df_5class(df, sample_count):
    ...
    sample_df = pd.concat([normal_random_df, secretion_random_df, resistant_random_df])
...
def add_preds_data(df, preds_df, sample_count):
    ...
    sample_df = pd.concat([df, virulence_random_df, toxin_random_df, anti_toxin_random_df])
...
if __name__ == "__main__":
    ...
    for i in range(num_runs):
        ...
        num_added_data = i*200 + 3000
        # for Training change the below files
        # main_train_csv_folder_path = "datasets/HMP_Swissprot_"+date+"_batch0_"+str(num_added_data)+"/"
        # main_output_save_folder_path = "results/HMP_Swissprot_"+date+"_batch0_"+str(num_added_data)+"/"
        main_train_csv_folder_path = "datasets/HMP_Swissprot_0508/"
        ...
        # save_dir = main_output_save_folder_path+"tremble_batch_results/"
        # # infer_df = pd.read_csv(dataset_dir + "Salmonella_typhimurium_(strainLT2).csv")
        # infer_df = pd.read_csv("datasets/TremBLE(UNIPROT)/Bacteria/batch_data/batch_0_5class_only_virulence_toxin_anti_toxin.csv")
        ...
        # print(infer_results)
        # infer_results.to_csv(save_dir + "batch_0_5class_only_virulence_toxin_anti_toxin.csv", index=False)
        ...
        # save_new_pth = "datasets/HMP_Swissprot_"+date+"_batch0_"+str(num_added_data+200)+"/"
        # results_save_new_pth = "results/HMP_Swissprot_"+date+"_batch0_"+str(num_added_data+200)+"/"
        ...
        # df = pd.read_csv("datasets/TremBLE(UNIPROT)/Bacteria/batch_data/batch_0_5class_only_virulence_toxin_anti_toxin.csv")
        
        # df_compare = pd.read_csv(preds_save_pth+"batch_0_5class_only_virulence_toxin_anti_toxin.csv")
        # # df_compare = df_compare.drop(['Unnamed: 0'], axis=1)
        ...
        # df_new.to_csv(preds_save_pth+"batch_0_5class_only_virulence_toxin_anti_toxin.csv", index=False)
        # df = pd.read_csv(preds_save_pth+"batch_0_5class_only_virulence_toxin_anti_toxin.csv")
        # # print(df)
        ...
        # df_1.to_csv(preds_save_pth+"batch_0_5class_only_virulence_toxin_anti_toxin.csv", index=False)
        
        # df_preds_matching = pd.read_csv(preds_save_pth+"batch_0_5class_only_virulence_toxin_anti_toxin.csv")
        ...
        # sampled_trainable_df = sample_trainable_df_5class(df_trainable, 200)
        ...
        # check_labels_count(df_preds_matching)
        # sampled_trainable_df_w_preds = add_preds_data(sampled_trainable_df, df_preds_matching, 200)
