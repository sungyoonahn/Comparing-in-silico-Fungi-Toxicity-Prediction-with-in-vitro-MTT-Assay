...
# This code is for all automized continuous self-supervised learning
...
def tsne_plot(features, true_labels, type, epoch):
    ...
    tsne_result = TSNE(n_components=2, perplexity=50).fit_transform(features)
...
def sample_trainable_df_4class(df, sample_count):
    ...
    adhesion_df = get_group_safely(df_by_labels,1)
    stress_df = get_group_safely(df_by_labels,2)
    acquisition_df = get_group_safely(df_by_labels,3)
    AMR_df = get_group_safely(df_by_labels,4)
    ...
    if len(adhesion_df)>=sample_count:
        adhesion_random_df = adhesion_df.sample(sample_count)
        empty_class_dict["adhesion"] = len(adhesion_random_df)
    else:
        adhesion_random_df = adhesion_df.sample(len(adhesion_df))
        empty_class_dict["adhesion"] = len(adhesion_df)
        
    if len(stress_df)>=sample_count:
        stress_random_df = stress_df.sample(sample_count)
        empty_class_dict["stress"] = 1
    else:
        stress_random_df = stress_df.sample(len(stress_df))
        empty_class_dict["stress"] = len(stress_df)
        
    if len(acquisition_df)>=sample_count:
        acquisition_random_df = acquisition_df.sample(sample_count)
        empty_class_dict["acquisition"] = 1
    else:
        acquisition_random_df = acquisition_df.sample(len(acquisition_df))
        empty_class_dict["acquisition"] = len(acquisition_df)
        
    if len(AMR_df)>=sample_count:
        AMR_random_df = AMR_df.sample(sample_count)
        empty_class_dict["AMR"] = 1
    else:
        AMR_random_df = AMR_df.sample(len(AMR_df))
        empty_class_dict["AMR"] = len(AMR_df)
        
    sample_df = pd.concat([normal_random_df, adhesion_random_df, stress_random_df, acquisition_random_df, AMR_random_df])
...
def add_preds_data(df, preds_df, sample_count, empty_class_dict):
    ...
    if empty_class_dict["adhesion"] == 0:
        adhesion_random_df = adhesion_df.sample(sample_count)
    elif empty_class_dict["adhesion"] == 1:
        adhesion_random_df = adhesion_df.sample(0)
    else:
        num_to_add = sample_count - int(empty_class_dict["adhesion"])
        adhesion_random_df = adhesion_df.sample(num_to_add)
    
    if empty_class_dict["stress"] == 0:
        stress_random_df = stress_df.sample(sample_count)
    elif empty_class_dict["stress"] == 1:
        stress_random_df = stress_df.sample(0)
    else:
        num_to_add = sample_count - int(empty_class_dict["stress"])
        stress_random_df = stress_df.sample(num_to_add)

    if empty_class_dict["acquisition"] == 0:
        acquisition_random_df = acquisition_df.sample(sample_count)
    elif empty_class_dict["acquisition"] == 1:
        acquisition_random_df = acquisition_df.sample(0)
    else:
        num_to_add = sample_count - int(empty_class_dict["acquisition"])
        acquisition_random_df = acquisition_df.sample(num_to_add)
        
    if empty_class_dict["AMR"] == 0:
        AMR_random_df = AMR_df.sample(sample_count)
    elif empty_class_dict["AMR"] == 1:
        AMR_random_df = AMR_df.sample(0)
    else:
        num_to_add = sample_count - int(empty_class_dict["AMR"])
        AMR_random_df = AMR_df.sample(num_to_add)
        


    sample_df = pd.concat([df, adhesion_random_df,stress_random_df, acquisition_random_df, AMR_random_df])
...
if __name__ == "__main__":
    ...
    num_runs = 1
    # initialize base data per class count
    num_per_class = 3000
    ...
    for i in range(num_runs):
        ...
        main_train_csv_folder_path = "sungyoon/env_project/datasets/fungi_train/Swissprot_"+date+"_"+str(num_per_class)+"/"
        main_output_save_folder_path = "sungyoon/env_project/results/fungi_train/Swissprot_"+date+"_"+str(num_per_class)+"/"
        ...
        infer_df = pd.read_csv("datasets/TREMBL(UNIPROT)/Fungi/uniprot_trembl_GO_split_labelled_trainable_classes.csv")
        ...
        print(infer_results)
        infer_results.to_csv(save_dir + "uniprot_trembl_GO_split_labelled_trainable_classes.csv", index=False)
        print("Done Inference ...")
        
        
        # 3 . Create New Dataset Part
        
        print("CURRENT NUMBER OF DATA PER CLASS: ",num_per_class)
        update_data_num = pow(2,(math.floor(math.log2(num_per_class/L))))
        if update_data_num < 1:
            update_data_num = 1

        update_data_num = int(update_data_num * L/10)
        # update_data_num = 70
        print("NUMBER OF DATA TO UPDATE: ",update_data_num)
        num_per_class = num_per_class+update_data_num
        # num_per_class = 100
        print("CURRENT NUMBER OF DATA PER CLASS: ",num_per_class)
        ...
        save_new_pth = "datasets/Swissprot_"+date+"_"+str(num_per_class)+"/"
        results_save_new_pth = "results/Swissprot_"+date+"_"+str(num_per_class)+"/"
        ...
        # file of all trembl data
        df = pd.read_csv("datasets/TREMBL(UNIPROT)/Fungi/uniprot_trembl_GO_split_labelled_trainable_classes.csv")
        
        df_compare = pd.read_csv(preds_save_pth+"uniprot_trembl_GO_split_labelled_trainable_classes.csv")
        # df_compare = df_compare.drop(['Unnamed: 0'], axis=1)
        ...
        df_new.to_csv(preds_save_pth+"uniprot_trembl_GO_split_labelled_trainable_classes.csv", index=False)
        df = pd.read_csv(preds_save_pth+"uniprot_trembl_GO_split_labelled_trainable_classes.csv")
        # print(df)
        ...
        df_1.to_csv(preds_save_pth+"uniprot_trembl_GO_split_labelled_trainable_classes_matching.csv", index=False)
        
        df_preds_matching = pd.read_csv(preds_save_pth+"uniprot_trembl_GO_split_labelled_trainable_classes_matching.csv")
        df_SH = pd.read_csv("datasets/TREMBL(UNIPROT)/Fungi/uniprot_swissprot_GO_split_labelled_trainable_classes.csv")
        ...
        check_labels_count(df_preds_matching)
        print("8. Label count where label matches predictions and where sequences are not included in Swissprot")
        check_labels_count(df_trainable)
        
        print("9. Label count of sampled data from swissprot")
        sampled_trainable_df, empty_class_dict = sample_trainable_df_4class(df_trainable, update_data_num)
        
        check_labels_count(sampled_trainable_df)
        
        print("10. Label count of sampled data from prediction data")
        check_labels_count(df_preds_matching)
        sampled_trainable_df_w_preds = add_preds_data(sampled_trainable_df, df_preds_matching, update_data_num, empty_class_dict)
